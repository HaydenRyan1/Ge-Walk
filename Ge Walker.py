...
# Tesseract config: single line of text, digits/comma only.
OCR_CONFIG = (r'--psm 7 'r'-c tessedit_char_whitelist=0123456789, 'r'-c classify_bln_numeric_mode=1')
# centre of minimap
MINIMAP_ORIGIN = (1837, 107)
# Minimap pixels per game coordinate unit.
SCALE = 2
# How close (in game tiles) we need to be to a waypoint before advancing to the next one, instead of requiring an exact match.
...
def coords():
    """
    Reads the player's current in-game coordinates via OCR.
    Returns (x, y) as ints, or None if OCR couldn't parse a coordinate pair.
    """
    ...
    if len(numbers) < 2:
        print(f"cant find coords (OCR read: '{text.strip()}')")
        return None
...
def walker(target):
    """
    Clicks the minimap position corresponding to the given target
    game-coordinate, based on the player's current OCR'd position.
    """
    current = coords()
    if current is None:
        return
...
def walk_path(path):
    ...
    for index in range(len(path) - 1):
        ...
        while True:
            ...
            if pos[0] == current_waypoint[0] and pos[1] == current_waypoint[1]:
